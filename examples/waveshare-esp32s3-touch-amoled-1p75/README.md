...
Once you confirm the wiring for **your** board (I²C on GPIO15/14, RST on GPIO2, TOUCH_INT on GPIO11 — only correct as-is for the 1.75" board), extend `touch_task` with your own gesture logic, or set an orientation/display mapping via `CST92xx::new(i2c, delay).with_reset(rst).with_config(config)`.
